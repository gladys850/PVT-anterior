...
<div class="block text-justify">
    ...
    <div>
        <b>SEPTIMA.- (DE LA FORMA DE PAGO Y OTRAS CONTINGENGIAS):</b> Para el cumplimiento estricto de la obligación (capital e intereses) el DEUDOR, autoriza expresamente a MUSERPOL practicar los descuentos respectivos de los haberes que percibe en forma mensual a través del Comando General de la Policía Boliviana.
    </div>
    <div>
    ...
    <div>
        <b>DECIMA CUARTA.- (CONFORMIDAD)</b> Nosotros, LA MUSERPOL, como ACREEDORES representado por su {{ $employees[0]['position'] }} Cnl. {{ $employees[0]['name'] }} y su {{ $employees[1]['position'] }} Lic. {{ $employees[1]['name'] }} por una parte y
        @if (count($lenders) == 1)
            @php ($lender = $lenders[0]->disbursable)
            @php ($male_female = Util::male_female($lender->gender))
