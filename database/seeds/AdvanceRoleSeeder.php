...
class AdvanceRoleSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $pay_permissions = ['print-payment-plan', 'print-payment-kardex-loan', 'show-loan', 'update-payment', 'show-payment','create-payment', 'show-payment-loan', 'disbursement-loan'];
        ...
        if ($module) {
            ...
            foreach ($sequence_roles as $role) {
                ...
                if (in_array($role['display_name'], ['Jefatura'])) {
                    ...
                    $role->syncPermissions(array_merge($leadership_permissions_treasury, $pay_permissions));
                } elseif (in_array($role['display_name'], ['Tesorería'])) {
                    $role->syncPermissions(array_merge($pay_permissions));
                }else {
