...
class Loan extends Model
{
    ...
    public $fillable = [
        ...
        'indebtedness_calculated_previus',
    ...
    {
        return $this->loan_affiliates()->withPivot('payment_percentage','payable_liquid_calculated', 'bonus_calculated', 'quota_previous','quota_treat','indebtedness_calculated','indebtedness_calculated_previus','liquid_qualification_calculated','contributionable_ids','contributionable_type')->whereGuarantor(true);
    }

    public function lenders()
    {
        return $this->loan_affiliates()->withPivot('payment_percentage','payable_liquid_calculated', 'bonus_calculated', 'quota_previous','quota_treat', 'indebtedness_calculated','indebtedness_calculated_previus','liquid_qualification_calculated','contributionable_ids','contributionable_type')->whereGuarantor(false);
    }
            ...
            if($loan_term <= $procedure_modality->loan_modality_parameter->maximum_term_modality && $loan_term >= $procedure_modality->loan_modality_parameter->minimum_term_modality){
                ...
                if($new_indebtedness_calculated<=$procedure_modality->loan_modality_parameter->debt_index){                      
                    if(count($this->guarantors)>0 || count($this->lenders) > 1){
                        if(($new_indebtedness_calculated <= Util::round2($this->indebtedness_calculated))|| ($new_indebtedness_calculated <= Util::round2($this->indebtedness_calculated_previus))){ 
                            foreach ($this->lenders  as $lender) {    
                                ...
                                if(($new_indebtedness_lender <= (float)$lender->pivot->indebtedness_calculated)||($new_indebtedness_lender <= (float)$lender->pivot->indebtedness_calculated_previus)){
                                    $validate = true;                
                                }else  $validate = false; 
                            }                         
                            if(count($this->guarantors)>0){   
                                foreach ($this->guarantors  as $guarantor) {    
                                    ...
                                    if($new_indebtedness_calculated_guarantor <= Util::round2((float)$guarantor->pivot->indebtedness_calculated) || ($new_indebtedness_calculated_guarantor <= (float)$guarantor->pivot->indebtedness_calculated_previus)){
                                        $validate = true;                                                                            
                                    }else $validate = false;                                  
                                }                                   
                            }  
